...
def save_figure(fig, filename, folder_path=FIGURES_WEEK4_DIR):
...
# =========================================================
# WEEK 4 SUPERVISED LEARNING VISUALIZATION FUNCTIONS
# =========================================================

def plot_confusion_matrix(cm, display_labels=['Retained (No)', 'Churned (Yes)'], filename="confusion_matrix.png"):
...
def plot_roc_curve(y_test, y_prob_dict, filename="roc_curve.png"):
...
def plot_precision_recall_curve(y_test, y_prob_dict, filename="precision_recall_curve.png"):
...
def plot_threshold_analysis(thresholds, precisions, recalls, f1s, filename="threshold_analysis.png"):
...
def plot_model_comparison(comparison_df, filename="model_comparison.png"):
    """Plots model comparison metrics bar chart."""
    metrics_melted = pd.melt(comparison_df, id_vars=['Model'], value_vars=['Accuracy', 'Precision', 'Recall', 'F1_Score', 'ROC_AUC'],
                             var_name='Metric', value_name='Value')

    fig, ax = plt.subplots(figsize=(10, 6))
    sns.barplot(data=metrics_melted, x='Metric', y='Value', hue='Model', palette='Set1', ax=ax)
    ax.set_title("Supervised Machine Learning Model Performance Comparison", fontsize=14, fontweight='bold')
...
def plot_top_coefficients(df_coef, top_n=12, filename="top_logistic_coefficients.png"):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.barplot(data=df_top, x='Coefficient', y='Feature', palette=colors, ax=ax)
